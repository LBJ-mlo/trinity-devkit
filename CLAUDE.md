...
**Think × Power × Skill** — 三库合一，42 个技能，4 层认知架构的统一开发平台。
...
Layer 2: 工程流程层   → using-agent-skills → 24个技能    "如何做好软件工程"
         ↕ 架构知识   → architecture-patterns           "代码该怎么组织"
                      → domain-driven-design            "领域该怎么建模"
...
3. **涉及项目结构/领域建模？** → 先调用 `architecture-patterns` 或 `domain-driven-design` 确定结构
4. **工程任务** → 按开发阶段用 agent-skills 决策树匹配技能
5. **执行中** → 按需调用 superpowers 工作流技能（brainstorming、subagent-driven-development 等）
...
| 🏛️ 项目结构设计 | `architecture-patterns` | domain-driven-design |
| 🧱 领域建模 (DDD) | `domain-driven-design` | architecture-patterns |
| 💡 需求头脑风暴 | `brainstorming` | idea-refine |
| 📝 写规范 | `spec-driven-development` | brainstorming + architecture-patterns |
| 📋 拆任务 | `planning-and-task-breakdown` | writing-plans + domain-driven-design |
...
| 项目结构 | `architecture-patterns`（选范式）+ `domain-driven-design`（DDD 落地） |
| 新系统设计 | `domain-driven-design`（战略→战术）+ `architecture-patterns`（分层规则） |
